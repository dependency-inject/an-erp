...
util.tableButton = function(h, params, type, text, onClick, permission) {
    if (permission !== undefined && !params.row[permission]) return null;
    return h('i-button', {
        props: { type: type, size: 'small' },
        style: { marginRight: '5px' },
        on: { click: () => { onClick(params.row); } }
    }, text);
};
...
util.formatTimestamp = function(timestamp, fmt) {
    return util.formatDate(new Date(timestamp), fmt);
};
