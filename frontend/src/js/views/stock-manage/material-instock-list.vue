        ...
        <div class="main-panel-content">
            <div class="operate-panel">
                <div class="pull-left operate-list" v-show="selectItems==''">
                    ...
                    <!-- 搜索框 -->
                    <i-input :placeholder="$t('component.PLEASE_INPUT')+$t('field.MATERIAL_INSTOCK.BILL_NO')+'/'+$t('field.MATERIAL_INSTOCK.FROM_PRINCIPAL')+'/'+$t('field.MATERIAL_INSTOCK.WAREHOUSE_PRINCIPAL')" v-model="vm.queryParameters.searchKey" @on-enter="search" style="width:280px"></i-input>
                ...
                <div class="pull-left operate-list" v-show="selectItems!=''">
                    ...
                    <!-- 审核 -->
                    <span class="label-btn" @click="audit(selectItems)" v-if="materialInstockAuditPermission"><icon type="checkmark"></icon>{{ $t('common.AUDIT') }}</span>
                    <!-- 反审核 -->
                    <span class="label-btn" @click="unaudit(selectItems)" v-if="materialInstockAuditPermission"><icon type="reply"></icon>{{ $t('common.UNAUDIT') }}</span>
                    <!-- 删除 -->
    ...
    computed: {
        ...
        columnList() {
            return [
                ...
                { title: this.$t('field.MATERIAL_INSTOCK.BILL_NO'), key: 'billNo', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_INSTOCK.BILL_TIME'), key: 'billTimeLocal', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_INSTOCK.FROM_PRINCIPAL'), key: 'fromPrincipalName', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_INSTOCK.WAREHOUSE_PRINCIPAL'), key: 'warehousePrincipalName', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_INSTOCK.MATERIAL_SOURCE'), key: 'materialSourceCn', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_INSTOCK.BILL_STATE'), key: 'billStateCn', sortable: 'custom' },
        ...
        stateList() {
            return [
                { value: -1, descript: this.$t('field.MATERIAL_INSTOCK_STATE.-1') },
                { value: 1, descript: this.$t('field.MATERIAL_INSTOCK_STATE.1') },
                { value: 2, descript: this.$t('field.MATERIAL_INSTOCK_STATE.2') },
                { value: 3, descript: this.$t('field.MATERIAL_INSTOCK_STATE.3') },
            ]
        }
    },
    methods: {
        ...
        async search() {
            ...
            let result = await materialInstockService.search(Object.assign({}, this.vm.queryParameters, { beginTime: beginTime, endTime: endTime }));
            if (result.status === 200) {
                var items = result.data.list;
                this.vm.queryParameters.total = result.data.total;
                items.forEach((item) => {
                    item.billStateCn = this.$t('field.MATERIAL_INSTOCK_STATE.' + Number(item.billState));
                    item.billTimeLocal = util.formatTimestamp(item.billTime, "yyyy-MM-dd hh:mm:ss");
                    item.materialSourceCn = this.$t('field.MATERIAL_INSTOCK_MATERIAL_SOURCE.' + Number(item.materialSource));
                    item['detailPermission'] = true;
                    if (item.billState === 1 && this.materialInstockRemovePermission)
        ...
        handleSort(data) {
            if (data.order === 'normal') {
                ...
                this.vm.queryParameters.sort = data.order;
            } else if (data.key === 'materialSourceCn') {
                this.vm.queryParameters.sortColumn = 'materialSource';
                this.vm.queryParameters.sort = data.order;
            } else {
        ...
        audit(selectItems) {
            ...
            this.$Modal.confirm({
                ...
                onOk: async () => {
                    let result = await materialInstockService.audit(idList)
                    if (result.status === 200) {
        ...
        unaudit(selectItems) {
            ...
            this.$Modal.confirm({
                content: this.$t('common.OPERATE_CONFIRM'),
                onOk: async () => {
                    let result = await materialInstockService.unaudit(idList)
                    if (result.status === 200) {
                        this.$Message.success(this.$t('common.OPERATE_SUCCESS'))
                        this.search()
                    } else {
