        ...
        <div class="main-panel-content">
            <div class="operate-panel">
                <div class="pull-left operate-list" v-show="selectItems==''">
                    ...
                    <!-- 搜索框 -->
                    <i-input :placeholder="$t('component.PLEASE_INPUT')+$t('field.MATERIAL_OUTSTOCK.BILL_NO')+'/'+$t('field.MATERIAL_OUTSTOCK.TO_PRINCIPAL')+'/'+$t('field.MATERIAL_OUTSTOCK.WAREHOUSE_PRINCIPAL')" v-model="vm.queryParameters.searchKey" @on-enter="search" style="width:280px"></i-input>
                ...
                <div class="pull-left operate-list" v-show="selectItems!=''">
                    ...
                    <!-- 审核 -->
                    <span class="label-btn" @click="audit(selectItems)" v-if="materialOutstockAuditPermission"><icon type="checkmark"></icon>{{ $t('common.AUDIT') }}</span>
                    <!-- 反审核 -->
                    <span class="label-btn" @click="unaudit(selectItems)" v-if="materialOutstockAuditPermission"><icon type="reply"></icon>{{ $t('common.UNAUDIT') }}</span>
                    <!-- 删除 -->
    ...
    computed: {
        ...
        columnList() {
            return [
                ...
                { title: this.$t('field.MATERIAL_OUTSTOCK.BILL_NO'), key: 'billNo', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_OUTSTOCK.BILL_TIME'), key: 'billTimeLocal', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_OUTSTOCK.TO_PRINCIPAL'), key: 'toPrincipalName', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_OUTSTOCK.WAREHOUSE_PRINCIPAL'), key: 'warehousePrincipalName', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_OUTSTOCK.MATERIAL_WHEREABOUTS'), key: 'materialWhereaboutsCn', sortable: 'custom' },
                { title: this.$t('field.MATERIAL_OUTSTOCK.BILL_STATE'), key: 'billStateCn', sortable: 'custom' },
        ...
        stateList() {
            return [
                { value: -1, descript: this.$t('field.MATERIAL_OUTSTOCK_STATE.-1') },
                { value: 1, descript: this.$t('field.MATERIAL_OUTSTOCK_STATE.1') },
                { value: 2, descript: this.$t('field.MATERIAL_OUTSTOCK_STATE.2') },
                { value: 3, descript: this.$t('field.MATERIAL_OUTSTOCK_STATE.3') },
            ]
        }
    },
    methods: {
        ...
        async search() {
            ...
            let result = await materialOutstockService.search(Object.assign({}, this.vm.queryParameters, { beginTime: beginTime, endTime: endTime }));
            if (result.status === 200) {
                var items = result.data.list;
                this.vm.queryParameters.total = result.data.total;
                items.forEach((item) => {
                    item.billStateCn = this.$t('field.MATERIAL_OUTSTOCK_STATE.' + Number(item.billState));
                    item.billTimeLocal = util.formatTimestamp(item.billTime, "yyyy-MM-dd hh:mm:ss");
                    item.materialWhereaboutsCn = this.$t('field.MATERIAL_OUTSTOCK_MATERIAL_WHEREABOUTS.' + Number(item.materialWhereabouts));
                    item['detailPermission'] = true;
                    if (item.billState === 1 && this.materialOutstockRemovePermission)
        ...
        handleSort(data) {
            if (data.order === 'normal') {
                ...
                this.vm.queryParameters.sort = data.order;
            } else if (data.key === 'materialWhereaboutsCn') {
                this.vm.queryParameters.sortColumn = 'materialWhereabouts';
                this.vm.queryParameters.sort = data.order;
            } else {
        ...
        audit(selectItems) {
            ...
            this.$Modal.confirm({
                ...
                onOk: async () => {
                    let result = await materialOutstockService.audit(idList)
                    if (result.status === 200) {
        ...
        unaudit(selectItems) {
            ...
            this.$Modal.confirm({
                content: this.$t('common.OPERATE_CONFIRM'),
                onOk: async () => {
                    let result = await materialOutstockService.unaudit(idList)
                    if (result.status === 200) {
                        this.$Message.success(this.$t('common.OPERATE_SUCCESS'))
                        this.search()
                    } else {
