                ...
                <i-form ref="formValidate" :model="item" :rules="rules" :label-width="90" inline>
                    <div class="chief-panel">
                        ...
                        <div class="panel-body">
                            <form-item :label="$t('field.PROINSTOCK.PROINSTOCK_ID')" prop="proInstockNo"><i-input v-model="item.proInstockNo"></i-input></form-item>
                        </div>
                    </div>
                    <div class="chief-panel">
                        <div class="panel-header">{{ $t('field.PROINSTOCK_INFO') }}</div>
                        <div class="panel-body">
                            <form-item :label="$t('field.PROINSTOCK.PROINSTOCK_TIME')" prop="proinstockTime"><i-input v-model="item.proinstockTime" :disabled=true></i-input></form-item>
                            <form-item :label="$t('field.PROINSTOCK.PROINSTOCK_PERSON')" prop="proInstockPerson"><i-input v-model="item.proInstockPerson"></i-input></form-item>
    ...
    methods: {
            ...
            this.$refs.formValidate.validate(async (valid) => {
                if (valid) {
                    if (this.$route.params.id === 'add') {
                        let result = await productinstockService.add(this.item);
                        ...
                        let result = await productinstockService.update(this.item);
                        if (result.status === 200) {
                            this.$Message.success(this.$t('common.SAVE_SUCCESS'));
                            this.initData();
                        } else {
        ...
        async getById() {
            ...
            if (result.status === 200) {
                ...
                //this.item.proinstockTime = new Date().toLocaleDateString(this.item.billTime);
