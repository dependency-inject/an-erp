        ...
        <div class="main-panel-content">
            <div class="operate-panel">
                <div class="pull-left operate-list" v-show="selectItems==''">
                    ...
                    <!-- 搜索框 -->
                    <i-input :placeholder="$t('component.PLEASE_INPUT')+$t('field.RETURN_MATERIAL.BILL_NO')+'/'+$t('field.RETURN_MATERIAL.FROM_PRINCIPAL')" v-model="vm.queryParameters.searchKey" @on-enter="search" style="width:280px"></i-input>
                    <!-- 日期选择框 -->
...
export default {
            ...
            return [
                ...
                { title: this.$t('field.OPERATE'), key: 'action', width: 200, render: (h, params) => {
                        return h('div', [ util.tableButton(h, params, 'primary', this.$t('common.DETAIL'), (row) => {
                            this.$router.push('/production-return/'+row[this.vm.identity])
    ...
    methods: {
        ...
        async search() {
            ...
            let result = await productionReturnService.searchBill(Object.assign({}, this.vm.queryParameters, { beginTime: beginTime, endTime: endTime }));
            if (result.status === 200) {
                ...
                items.forEach((item) => {
                    ...
                    if (item.billState === 1 && this.productionReturnRemovePermission)
        ...
        audit(selectItems) {
            ...
            this.$Modal.confirm({
                ...
                onOk: async () => {
                    let result = await productionReturnService.auditBill(idList)
                    if (result.status === 200) {
        ...
        unaudit(selectItems) {
            ...
            this.$Modal.confirm({
                content: this.$t('common.OPERATE_CONFIRM'),
                onOk: async () => {
                    let result = await productionReturnService.unauditBill(idList)
                    if (result.status === 200) {
                        this.$Message.success(this.$t('common.OPERATE_SUCCESS'))
                        this.search()
                    } else {
        ...
        remove(selectItems) {
            ...
            this.$Modal.confirm({
                ...
                onOk: async () => {
                    let result = await productionReturnService.deleteBill(idList)
                    if (result.status === 200) {
