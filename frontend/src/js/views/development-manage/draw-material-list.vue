        ...
        <div class="main-panel-content">
            <div class="operate-panel">
                ...
                <div class="pull-left operate-list" v-show="selectItems!=''">
                    ...
                    <!-- 审核 -->
                    <span class="label-btn" @click="audit(selectItems)" v-if="developmentDrawAuditPermission"><icon type="checkmark"></icon>{{ $t('common.AUDIT') }}</span>
                    <!-- 反审核 -->
                    <span class="label-btn" @click="unaudit(selectItems)" v-if="developmentDrawAuditPermission"><icon type="reply"></icon>{{ $t('common.UNAUDIT') }}</span>
                    <!-- 删除 -->
                    <span class="label-btn" @click="remove(selectItems)" v-if="developmentDrawRemovePermission"><icon type="trash-a"></icon>{{ $t('common.REMOVE') }}</span>
                </div>
                <div class="button-list pull-right">
                    <i-button class="operate-btn" type="primary" shape="circle" @click="$router.push('/development-draw/add')">{{ $t('common.ADD') }}</i-button>
                </div>
            ...
            return [
                ...
                { title: this.$t('field.OPERATE'), key: 'action', width: 200, render: (h, params) => {
                        return h('div', [ util.tableButton(h, params, 'primary', this.$t('common.DETAIL'), (row) => {
                            this.$router.push('/development-draw/'+row[this.vm.identity]) 
    ...
    methods: {
        ...
        audit(selectItems) {
            ...
            this.$Modal.confirm({
                ...
                onOk: async () => {
                    let result = await developmentDrawService.auditBill(idList)
                    if (result.status === 200) {
        ...
        unaudit(selectItems) {
            ...
            this.$Modal.confirm({
                content: this.$t('common.OPERATE_CONFIRM'),
                onOk: async () => {
                    let result = await developmentDrawService.unauditBill(idList)
                    if (result.status === 200) {
                        this.$Message.success(this.$t('common.OPERATE_SUCCESS'))
                        this.search()
                    } else {
        ...
        remove(selectItems) {
            ...
            this.$Modal.confirm({
                ...
                onOk: async () => {
                    let result = await developmentDrawService.deleteBill(idList)
                    if (result.status === 200) {
