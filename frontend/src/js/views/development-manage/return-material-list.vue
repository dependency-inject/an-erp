		...
		<div class="main-panel-content">
			<div class="operate-panel">
				<div class="pull-left operate-list" v-show="selectItems==''">
					...
					<div><Input clearable type="text" v-model="searchKey.fromPrincipal" :placeholder="$t('field.RETURN_MATERIAL.FROM_PRINCIPAL')" @on-enter="selectItems=[];search()"></Input></div>
				...
				<div class="button-list pull-right">
					<Button  type="primary" @click="$router.push('/development-return/add')">{{ $t('common.ADD') }}</Button>
				</div>
    ...
    export default {
        ...
        computed: {
            ...
            columnsList() {
                return [
                    ...
                    { title: this.$t('field.RETURN_MATERIAL.BILL_NO'), key: 'billNo' },
                    { title: this.$t('field.RETURN_MATERIAL.FROM_PRINCIPAL'), key: 'fromPrincipal' },
                    { title: this.$t('field.RETURN_MATERIAL.RETURN_REASON'), key:'returnReason' },
                    { title: this.$t('field.RETURN_MATERIAL.BILL_STATE'), key: 'billState' },
                    { title: this.$t('field.RETURN_MATERIAL.BILL_TIME'), key: 'billTime' },
                    { title: this.$t('field.RETURN_MATERIAL.REMARK'), key: 'remark' },
                    { title: this.$t('field.OPERATE'), key: 'action', render: (h, params) => {
                            return h('div', [
                                h('Button', {
                                    ...
                                    on: { click: () => {
                                            this.$router.push('/development-return/' + params.row[this.vm.identity])
                                        }}
        ...
        methods: {
            ...
            async search() {
                ...
                let result = await developmentReturnService.searchBill(JSON.stringify(this.searchKey))
                console.log(result)
                if (result.status === 200) {
                    ...
                    items.forEach((item) => {
                        ...
                        if (item.returnReason === 2) {
                            item.returnReason = this.$t('field.RETURN_REASON.REASON2')
                        }
            ...
            remove(selectItems) {
                ...
                this.$Modal.confirm({
                    ...
                    onOk: async () => {
                        let result = await developmentReturnService.deletebills(idList)
                        if (result.status === 200) {
