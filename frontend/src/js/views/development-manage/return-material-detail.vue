                ...
                <i-form ref="formValidate" :model="item" :rules="rules" :label-width="90" inline>
                    <div class="chief-panel">
                        ...
                        <div class="panel-body">
                            <div>
                                <div class="info-item"><label>{{ $t('field.RETURN_MATERIAL.BILL_NO') }}： </label>{{ item.billNo }}</div>
                                <div class="info-item"><label>{{ $t('field.RETURN_MATERIAL.FROM_PRINCIPAL') }}： </label>{{ item.fromPrincipalName }}</div>
                                <div class="info-item"><label>{{ $t('field.RETURN_MATERIAL.WAREHOUSE_PRINCIPAL') }}： </label>{{ item.warehousePrincipalName }}</div>
                                <div class="info-item"><label>{{ $t('field.RETURN_MATERIAL.BILL_TIME') }}： </label>{{ item.billTimeLocal }}</div>
                                <div class="info-item"><label>{{ $t('field.RETURN_MATERIAL.RETURN_REASON') }}： </label>{{ item.returnReasonCn }}</div>
                                <div class="info-item"><label>{{ $t('field.RETURN_MATERIAL.BILL_STATE') }}： </label>{{ item.billStateCn }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="chief-panel">
        ...
        methods: {
            ...
            setDefault() {
                this.item = {
                    ...
                    returnReasonCn: this.$t('field.RETURN_REASON.REASON2'),
                    remark: '',
                    materialList: []
                }
            },
            async getById() {
                ...
                if (result.status === 200) {
                    ...
                    this.item.returnReasonCn = this.$t('field.RETURN_REASON.REASON' + Number(this.item.returnReason));
                }
            },
            save() {
            ...
            async getMaterialList() {
                let result = await developmentReturnService.getMaterialList();
                if (result.status === 200) {
