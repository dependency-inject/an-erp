...
export default {
    ...
    computed: {
        title() {
            return {
                totalAmount: this.$t('field.MATERIAL_STOCK.MATERIAL_AMOUNT'),
                orderedAmount: this.$t('field.MATERIAL_STOCK.MATERIAL_ORDERED'),
                leftAmount: this.$t('field.MATERIAL_STOCK.MATERIAL_LEFT')
            };
        }
    },
    methods: {
        ...
        async getStockOrderByField(field) {
            let result = await stockService.materialSearch(Object.assign({ current: 1, limit: this.itemLimit, sortColumn: field, sort: 'desc' }, this.queryParameters));
